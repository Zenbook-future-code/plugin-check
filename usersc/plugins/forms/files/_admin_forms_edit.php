...
if(!empty($_POST['deleteValidation'])){
  ...
  if(isValidValidation($delete)){
    ...
    // Validate field is numeric to prevent SQL injection
    if(!is_numeric($field) || (int)$field != $field || $field <= 0){
...
if($checkC < 1 && is_numeric($edit)){
  ...
  $check = $checkQ->first();
  // Validate form name from database to prevent SQL injection
...
if(!empty($_GET['switchto'])){
  // Validate field is numeric before using in update
  if(!is_numeric($field) || (int)$field != $field || $field <= 0){
    die("Invalid field ID");
  }
  $field = (int)$field;
